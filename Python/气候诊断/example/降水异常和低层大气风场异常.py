...
prec = ds['pre'].loc[:,10:,-90:130]
lat = ds['lat'].loc[10:]
lon = ds['lon'].loc[-90:130]

uwnd = du['uwnd'].loc[:,850,:10,-90:130]
vwnd = dv['vwnd'].loc[:,850,:10,-90:130]

ulat = du['lat'].loc[:10]
ulon = du['lon'].loc[-90:130]
...
for myseason in ["SON","OND","NDJ"]:
   ...
   # plot significant regions
	...
	uvflux_not_sig = ax.quiver(ulon[::2], ulat[::2], u_reg_xr[::2,::2], v_reg_xr[::2,::2], transform=ccrs.PlateCarree(), pivot='mid', width=0.0028, scale=20.0, headwidth=4, color="gray")
	uvflux_sig = ax.quiver(ulon[::2], ulat[::2], u_sig_xr[::2,::2], v_sig_xr[::2,::2], transform=ccrs.PlateCarree(), pivot='mid', width=0.0028, scale=20.0, headwidth=4, color="k")
	
	uvflux_sig_key = ax.quiverkey(uvflux_sig, 0.95, -0.18, 1.0, "1.0", color="black", fontproperties=fontproperties)
